export const USER = {
  firstName: 'Japheth',
  lastName: 'Oruko',
  displayName: 'Japheth Oruko',
  username: 'OJ254',
  primaryTitle:
    'Lead Product Designer, Lead Frontend Developer, & AI-assisted Product Builder',
  shortBio:
    'I help teams turn early product ideas into usable, scalable digital experiences across user-centric UI/UX, product thinking, design systems, documentation, and React/Next.js frontend implementation.',
  hero: 'Product Designer and Frontend Developer helping teams move from ambiguous ideas to user-centered, scalable digital products.',
  currentCompany: 'PixeSci',
  currentCompanyWebsite: 'https://pixesci.com/',
  currentCompanyApplication: 'https://pixesci.com/',
  currentRole: 'Lead Product Designer and Frontend Developer',
  currentRoleNote:
    'Current work spans the PixeSci website, application redesign, design system, frontend direction, and AI-assisted design-to-code delivery.',
  location: 'Nairobi, Kenya',
  email: 'japhethoruko@gmail.com',
  phone: '+254 758 750674',
  calendly: 'https://calendly.com/japhethoruko',
  github: 'https://github.com/OJ254',
  linkedin: 'https://www.linkedin.com/in/japheth-oruko-b5b9a4301/',
  behance: 'https://www.behance.net/japhethoruko',
  spotify: 'https://open.spotify.com/user/vdfj74s06581jrp8n5rr3ic2k',
  resume: '/documents/japheth-oruko_resume.pdf',
  languages: ['English fluent', 'Swahili native'],
  availability: 'Full-time and contract available',
  timeZone: 'Africa/Nairobi',
  keywords: [
    'Japheth Oruko',
    'Product Designer',
    'UI UX Designer',
    'Product Manager',
    'Frontend Developer',
    'Lead Frontend Developer',
    'Next.js Developer',
    'React Developer',
    'Claude',
    'Codex',
    'MUI',
    'DaisyUI',
    'shadcn/ui',
    'Tailwind CSS',
    'Figma Designer',
    'Nairobi',
    'Kenya',
    'Startup Product Designer',
    'Design Systems',
  ],
  roles: [
    'Product Designer',
    'Product Manager',
    'Frontend Developer',
    'UI/UX Consultant',
    'Lead Product Designer at PixeSci',
    'Lead Frontend Developer at PixeSci',
    'React.js and Next.js specialist',
    'MUI, DaisyUI, shadcn/ui, and Tailwind builder',
    'Claude and Codex power user',
    'Zero-to-one product builder',
    'Design systems and MVP delivery',
  ],
} as const;
