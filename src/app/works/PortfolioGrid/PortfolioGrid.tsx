...
const projects: Project[] = [
...

  {
    ...
    description:
      'PROBLEM: As the Event Discovery Platform scales, the primary UX challenge becomes cognitive overload. With webinars, meetups, conferences, and workshops increasing in volume, users risk missing relevant events due to poor scannability, inconsistent layouts, or hidden status cues. Users need to quickly answer four key questions within seconds: Is this event relevant to me? Where and how will it happen? Does it cost anything? Am I already registered? Without a clear hierarchy, featured events lose prominence, registered events are hard to track, and decision-making becomes slow—undermining the platform’s core value of effortless discovery.\n' +
      '\n' +
      'SOLUTION: The design introduces a reusable Event Card component paired with a structured Events Listing Page to streamline discovery and comparison. Each Event Card prioritizes visual hierarchy: a banner image establishes context, followed by a bold title, a concise two-line description, and clearly grouped metadata (location, mode, reach, and pricing). Badges (Online, Free, Featured, Registered) are visually consistent and immediately scannable. On the Events Listing Page, a hero section with search and filters anchors user intent, while a featured upcoming event is elevated using a larger layout to capture attention. Below it, a responsive grid of Event Cards ensures scalability and consistency as the catalog grows, with a clear empty state to handle edge cases gracefully.\n' +
      '\n' +
      'REASONING: The UX logic is driven by decision speed and system thinking. Users scan vertically and left-to-right, so critical information is surfaced in predictable positions across all cards. Badges reduce reading effort by translating attributes into recognizable visual signals, while consistent spacing and typography reinforce pattern recognition. The separation between a featured event and the main grid leverages visual contrast to guide attention without disrupting flow. By designing the Event Card as a modular component, the platform supports reuse across listings, search results, and future contexts, ensuring long-term scalability. Overall, the solution balances clarity, efficiency, and flexibility—allowing users to understand an event’s relevance almost instantly while keeping the interface clean and extensible.',
    primaryLink:
